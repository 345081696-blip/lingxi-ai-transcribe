...
// Windows: 启用高 DPI 支持（Electron 默认已启用，显式声明避免歧义）
...
function createWindow() {
  // 根据主屏逻辑分辨率动态计算窗口大小，使不同 DPI 屏幕上视觉比例一致
  // 目标：占屏幕可用区的约 85%，但不超过 Mac 版理想尺寸 1180x780，也不小于最小可用尺寸
  let winW = 1180;
  let winH = 780;
  try {
    const area = screen.getPrimaryDisplay().workAreaSize;
    if (area && area.width && area.height) {
      winW = Math.max(960, Math.min(1180, Math.round(area.width * 0.85)));
      winH = Math.max(640, Math.min(780, Math.round(area.height * 0.85)));
    }
  } catch (e) { /* 使用默认尺寸 */ }
  ...
  mainWindow.webContents.on('did-finish-load', () => {
    try { mainWindow.webContents.setZoomFactor(0.75); } catch (e) {}
  });
...
function createWidgetWindow() {
  ...
  widgetWindow.webContents.on('did-finish-load', () => {
    try { widgetWindow.webContents.setZoomFactor(0.75); } catch (e) {}
  });
...
function resolveFfmpeg() {
  const isWin = process.platform === 'win32';
  // 优先使用打包随附的 ffmpeg（extraResources 中的 ffmpeg/ffmpeg.exe），
  // 否则系统 PATH 里没有 ffmpeg 时区域裁剪等步骤会静默失败。
  const bundled = isWin
  ...
  const candidates = [
    ...
    bundled,
    isWin ? 'ffmpeg.exe' : '/opt/homebrew/bin/ffmpeg',
    isWin ? 'ffmpeg' : '/usr/local/bin/ffmpeg',
    'ffmpeg'
  ].filter(Boolean);
...
function resolveOpenClawCommand(customCommand = '') {
  ...
  const isWin = process.platform === 'win32';
  const oc = isWin ? 'openclaw.exe' : 'openclaw';
  const candidates = [
    process.env.OPENCLAW_BIN,
    isWin ? 'openclaw.exe' : '/opt/homebrew/bin/openclaw',
    isWin ? 'openclaw' : '/usr/local/bin/openclaw',
    oc
  ].filter(Boolean);
  return candidates.find((candidate) => candidate === 'openclaw' || candidate === 'openclaw.exe' || fs.existsSync(candidate)) || oc;
}
...
ipcMain.handle('crop-recording', async (_event, payload = {}) => {
  const { filePath, x, y, width, height } = payload || {};
  if (!filePath || !fs.existsSync(filePath)) throw new Error('裁剪源文件不存在。');
  const num = (value, fallback = 0) => Math.max(0, Math.round(Number(value) || fallback));
  const cw = num(width, 0);
  const ch = num(height, 0);
  if (!cw || !ch) return { filePath };
  const cx = num(x, 0);
  const cy = num(y, 0);
  const parsed = path.parse(filePath);
  const outExt = parsed.ext === '.webm' ? '.mp4' : parsed.ext;
  const outName = `${path.basename(parsed.name, parsed.ext)}_crop${outExt}`;
  const outPath = uniquePath(path.join(parsed.dir, outName));
  try {
    ...
    throw new Error(`区域裁剪失败：${error.message || String(error)}`);
  }
  if (fs.existsSync(outPath) && fs.statSync(outPath).size > 0) {
    try { fs.unlinkSync(filePath); } catch (_e) { /* 保留原始文件 */ }
    return { filePath: outPath, cropped: true };
  }
  return { filePath };
});
